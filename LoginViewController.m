...
- (IBAction)loginButtonAction:(id)sender {
    ...
    if (netStatus == NotReachable)
    {
        ...
        if ([self.nameText.text isEqualToString:@""]  ) {
            if ([self.nameText.text isEqualToString:@""]) {
                self.nameText.backgroundColor = [UIColor redColor];
                self.nameText.background= [UIImage imageNamed:@"emptyBG.png"];
            }
            ...
            [body appendData:[[NSString stringWithFormat:@"%@",self.nameText.text] dataUsingEncoding:NSUTF8StringEncoding]];
            [body appendData:[[NSString stringWithFormat:@"\r\n--%@\r\n",boundary] dataUsingEncoding:NSUTF8StringEncoding]];
